...
public class ArogyaHospitalManagement{
...
// Description on code
// The ArogyaKendra Hospital  Management is a project that manages all patients details.instead of writing it manually we can manage patient data through this application  .the patient details include name,age,adhar number,contact number etc. and each patient should be provided with the different id. This project includes  modules like adding details of patient when the new patient is joined to the hospital and whenever we  want to search a patient details we can enter the patient id it should provide details of particular patient based on city or state. and we created a method to display the list of all patients from particular city.here we  creating a method to find patient based on age group.the module  also include a patient recovered .this patient should update the patients status.it also includes a delete option for patient who is not present.here we are Implementing exceptions that occur during execution and also the nullPointerException when user details is empty.