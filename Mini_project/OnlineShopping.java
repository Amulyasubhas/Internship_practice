...
// Description on code
// The is a mini project in Java that shows simple online shopping system.instead of doing it manually to add products or remove products by creating application using program it makes work easier . The program is designed to allow users to view products, add them to a shopping cart, and check out.It defines an interface called "Product" that defines the basic properties of a product such as the name, description, price, and quantity. Two classes "ElectronicProduct" and "ClothingProduct" implement the "Product" interface, and have additional properties specific to each product type.A class called "ShoppingCart" is created to store the products selected by the user, and a class called "Checkout" handles the payment and shipping of the products in the shopping cart.  we use Custom exceptions are used to handle cases such as insufficient stock or invalid payment information.

// Additional features such as search, sorting, and filtering can be implemented as well. The system can be expanded to include more product categories and payment options.The project utilizes inheritance, interfaces, overriding, collections, and custom exceptions to implement the system.